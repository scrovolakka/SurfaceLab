...
rewrite individual control cages. The surface Root null IS the pivot. Rig
creation seats the Root on the origin the surface's current Rotation Origin mode
implies, then switches the mode to Custom and binds the origin percentages to
the point Nulls so the plug-in's rotation/scale origin tracks the Root exactly,
wherever it is. Rotation and Scale pivot on the Root in the renderer and the
Null hierarchy alike, and dragging the Root moves the pivot with it -- no need
to re-run the script or keep the Rotation Origin mode in sync by hand. The
binding uses the surface's persistent ID
